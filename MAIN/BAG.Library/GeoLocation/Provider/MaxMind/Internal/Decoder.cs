namespace BAG.Framework.Geolocation.Provider.MaxMind.Internal
{
    using System;
    using System.IO;
    ...
    internal class Decoder
    {
        ...
        {
            if (offset >= this._stream.Value.Length)
                throw new InvalidDatabaseException("The MaxMind DB file's data section contains bad data: "
                                                    + "pointer larger than the database.");

            byte ctrlByte = this.ReadOne(offset);
            offset++;

            var type = this.FromControlByte(ctrlByte);
            ...
            if (type == ObjectType.Pointer)
            {
                ...
                var result = this.Decode(Convert.ToInt32(pointer));
            ...
            {
                int nextByte = this.ReadOne(offset);
        ...
        private Result DecodeByType(ObjectType type, int offset, int size)
        {
            ...
            byte[] buffer = this.ReadMany(offset, size);
            ...
            switch (type)
            {
                ...
                case ObjectType.Boolean:
                    return new Result(this.DecodeBoolean(size), offset);
                case ObjectType.Utf8String:
                    return new Result(this.DecodeString(buffer), newOffset);
                case ObjectType.Double:
                    return new Result(this.DecodeDouble(buffer), newOffset);
                case ObjectType.Float:
                    return new Result(this.DecodeFloat(buffer), newOffset);
                case ObjectType.Bytes:
                    return new Result(new JValue(buffer), newOffset);
                case ObjectType.Uint16:
                    return new Result(this.DecodeIntegerToJValue(buffer), newOffset);
                case ObjectType.Uint32:
                    return new Result(this.DecodeLong(buffer), newOffset);
                case ObjectType.Int32:
                    return new Result(this.DecodeIntegerToJValue(buffer), newOffset);
                case ObjectType.Uint64:
                    return new Result(this.DecodeUInt64(buffer), newOffset);
                case ObjectType.Uint128:
                    return new Result(this.DecodeBigInteger(buffer), newOffset);
                default:
        ...
        private int[] SizeFromCtrlByte(byte ctrlByte, int offset)
        {
            ...
            if (size == 29)
            {
                byte[] buffer = this.ReadMany(offset, bytesToRead);
        ...
        private int DecodePointer(int ctrlByte, int offset, out int outOffset)
        {
            ...
            byte[] buffer = this.ReadMany(offset, pointerSize);
            int packed = DecodeInteger(b, buffer);
            outOffset = offset + pointerSize;
            return packed + this._pointerBase + this._pointerValueOffset[pointerSize];
        }
